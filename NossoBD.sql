...
CREATE OR REPLACE FUNCTION FN_LIMPAR_DADOS_PACIENTE()
    RETURNS TRIGGER AS $$
BEGIN
    -- antes de apagar executa a função
    -- ele faz um por cpf
...
DECLARE
    ...
    v_id_sala INT;
BEGIN
    -- Pega o codigo da entrada e ve o paciente e o hospital onde deu entrada
    ...
    -- Se a sala estiver livre permite, n deve dar erro pois a view apaga se livre = false
     IF EXISTS (SELECT 1 FROM SALA WHERE ID = v_id_sala AND LIVRE = FALSE) THEN
        RAISE EXCEPTION 'Sala % está ocupada.', p_numero_sala;
     END IF;

    --Se achou a sala ele marca a sala como ocupada
     UPDATE SALA SET LIVRE = FALSE WHERE ID = v_id_sala;

    --Registra uma nova doença PARA AQUELE PACIENTE
    ...
    --  pega o profissional e vê qual medico ele corresponde
    ...
    -- pega o tipo de proc do combobox (raio-xx etc)
    ...
    -- cria o novo procedimento (a entrada n fez um raio-x)
